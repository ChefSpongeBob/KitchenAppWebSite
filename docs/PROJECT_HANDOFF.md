...
- Active phase: `20. Apple App Store readiness`
- Status: Phase 20 Apple repo readiness is validated locally. Remaining Apple blockers are external/manual: create App Store Connect app record, create Apple subscription products, upload TestFlight build, enter privacy details, provide review demo account, add screenshots/listing metadata, and run sandbox IAP on real iOS.
- Current pass: Phase 20 Apple App Store readiness. Public support, privacy, account deletion, subscription purchase/restore/management surfaces, Apple app identity, and Apple readiness guard are validated.
